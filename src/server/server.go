...
func (this *MockHttpServer) getEndPoint(id string) *EndPoint {
	var result *EndPoint = nil
	for e := this.endPoints.Front(); e != nil; e = e.Next() {
		if e.Value.(*EndPoint).id == id {
			result = e.Value.(*EndPoint)
			break
		}
	}
	return result
...
func HandlerEndPoint(w http.ResponseWriter, r *http.Request) {
	if r.Method == "POST" {
		...
		if err == nil {
			...
			ep := NewEndPoint("test", "testaddr", 8081, handler, ln)
			go http.Serve(ln, ep)
